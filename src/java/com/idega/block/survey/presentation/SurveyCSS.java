		...
		try {
			...
			if(question.getAnswerType()==SurveyBusinessBean.ANSWERTYPE_TEXTAREA){
				...
				if(iter.hasNext()){
					...
					li.add(new HiddenInput(PRM_SELECTION_PREFIX+question.getPrimaryKey().toString(),answer.getPrimaryKey().toString()));
					li.add(getAnswerTextArea(answer.getPrimaryKey()));
				}
			} else {
				Lists lists = new Lists();
				aL.add(lists);
