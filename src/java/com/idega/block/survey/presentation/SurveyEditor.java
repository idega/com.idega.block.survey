...
/**
 * Title: SurveyEditor Description: Copyright: Copyright (c) 2003 Company: idega
 * Software
 * 
...
public class SurveyEditor extends FolderBlock {
		...
		try {
			...
			if (surveys != null && !surveys.isEmpty()) {
				...
				while (iter.hasNext()) {
					...
					// String.class);
					// addAttribute(getColumnNameDescription(), "Description", true, true,
					// String.class);
					// addAttribute(getColumnNameStartTime(), "Begins", true, true,
					// Timestamp.class);
					// addAttribute(getColumnNameEndTime(), "Ends", true, true,
					// Timestamp.class);
	...
	private Object storeSurvey(IWContext iwc) throws IDOAddRelationshipException, CreateException, FinderException, IDORemoveRelationshipException, IDOLookupException, EJBException, RemoteException, RemoveException {
			...
			for (int i = 0; i < questions.length && i < answerType.length; i++) {
				if (!"".equals(questions[i]) && !"".equals(answerType[i])) {
					...
					if (NumberOfQuestionIDs <= i) {
						...
						// prmVector.remove(new
						// Parameter(PRM_QUESTION_IDS+PRM_MAINTAIN_SUFFIX,questionIDs[i]));
						// } else{
						...
						for (int j = 0; j < answers.length; j++) {
							if (answers[j] != null && !"".equals(answers[j])) {
								if (numberOfAnswerIDs <= j) {
									...
									// if(answers[j] != null && !"".equals(answers[j]) && type !=
									// ANSWERTYPE_TEXTAREA){
									// prmVector.remove(new
									// Parameter(PRM_ANSWER_IDS+(i+1)+PRM_MAINTAIN_SUFFIX,answerIDs[j]));
		...
		for (Iterator dAns = this._delAnswer.iterator(); dAns.hasNext();) {
			...
			// TMP //TODO
			// prmVector.add(new
			// Parameter(PRM_ANSWER_IDS+questionNumber+PRM_MAINTAIN_SUFFIX,sAnswerID));
	...
	private void processDeleteParameters(IWContext iwc) {
		...
		if (deletedAnswers != null) {
			for (int i = 0; i < deletedAnswers.length; i++) {
				...
				// prmVector.add(new
				// Parameter(PRM_ANSWER_IDS+questionNumber+PRM_MAINTAIN_SUFFIX,deletedAnswers[i]));
			}
		}
	...
	private void processNumberOfQuestionsPRM(IWContext iwc) {
		...
		if ((this._lastState == STATE_ONE && this._state == STATE_TWO)) { // || (_lastState ==
																														// STATE_TWO &&
																														// _state==STATE_ONE)){
			String[] questions = (String[]) this._prmValues.get(PRM_QUESTION);
			if (questions != null && questions.length != 0) {
				...
				// Warning
				// System.err.println(this.getClassName()+"[Warning]: Trying to go
				// forward without defining any question");
			}
		}
		else {
	...
	private PresentationObject getStateOne(IWContext iwc) {
		...
		// SubmitButton saveButton = new
		// SubmitButton(_iwrb.getLocalizedString("save"," Save
		// "),PRM_ACTION,String.valueOf(ACTION_SAVE));
		// setStyle(saveButton);
	...
	private PresentationObject getStateTwo(IWContext iwc) {
		...
		// SubmitButton CancelButton = new
		// SubmitButton(_iwrb.getLocalizedString("cancel"," Cancel
		// "),PRM_ACTION,String.valueOf(ACTION_CANCEL));
